...
if ($creds_result && $fetch_creds = mysqli_fetch_assoc($creds_result)) {
    ...
    // Fetch Google Ads IDs from database
    $google_ads_id = isset($fetch_creds['google_ads_id_1']) ? $fetch_creds['google_ads_id_1'] : '';
    $google_ads_label = isset($fetch_creds['google_ads_purchase_label_1']) ? $fetch_creds['google_ads_purchase_label_1'] : '';
...
<head>
    ...
    <!-- Google tag (gtag.js) loaded dynamically from database -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=<?php echo htmlspecialchars($google_ads_id); ?>"></script>
    <script>
      window.dataLayer = window.dataLayer || [];
      function gtag(){dataLayer.push(arguments);}
      gtag('js', new Date());

      gtag('config', '<?php echo htmlspecialchars($google_ads_id); ?>');
    </script>
    
        <?php if (!empty($google_ads_label)): ?>
        <!-- Event snippet for Purchase conversion page -->
        <script>
          gtag('event', 'conversion', {
              'send_to': '<?php echo htmlspecialchars($google_ads_id); ?>/<?php echo htmlspecialchars($google_ads_label); ?>',
              'value': '<?php echo $final_payable_amount; ?>',
              'currency': 'INR',
