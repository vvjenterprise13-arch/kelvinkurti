...
$brandName = 'YourStore'; // Default brand name
$brand_key = 'brand_name';
$setting_stmt = $conn->prepare("SELECT setting_value FROM settings WHERE setting_key = ?");
$setting_stmt->bind_param("s", $brand_key);
$setting_stmt->execute();
$setting_result = $setting_stmt->get_result();
if ($fetch_setting = $setting_result->fetch_assoc()) {
    $brandName = htmlspecialchars($fetch_setting['setting_value']);
}
$setting_stmt->close();
...
<html lang="en-IN">
<head>
    <title>Add Delivery Address - YourStore</title>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <style>
        :root { --primary-color: #2874f0; --accent-color: #fb641b; }
        body { background-color: #ffffffff; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif; }
        ...
        .header-brand { font-size: 14px; color: #fff; margin-left: auto; }
        .progress-stepper { display: flex; justify-content: space-between; padding: 10px 5px; background-color: #fff; border-bottom: 1px solid #f0f0f0; }
        ...
        .step.active .step-circle { background-color: var(--primary-color); color: #fff; border-color: var(--primary-color); }
        ...
        .form-section { padding: 16px; padding-bottom: 150px; }
        .form-floating > .form-control { height: 50px; padding-top: 1.3rem; font-size: 14px; }
        .form-floating > label { padding: 0.8rem 0.75rem; font-size: 14px; color: #6c757d; }
        .form-control:focus { border-color: var(--primary-color); box-shadow: 0 0 0 0.25rem rgba(40, 116, 240, 0.25); }
        ...
        .address-type-btn { border: 1px solid #dcdcdc; padding: 8px 16px; border-radius: 20px; cursor: pointer; display: flex; align-items: center; gap: 8px; font-size: 14px; }
        .address-type-options input[type="radio"]:checked + .address-type-btn { border-color: var(--primary-color); background-color: #f0f5ff; font-weight: 500; color: var(--primary-color); }
        .page-footer { background: #fff; border-top: 1px solid #e0e0e0; position: fixed; bottom: 0; width: 100%; max-width: 1248px; left: 50%; transform: translateX(-50%); padding: 12px 16px; }
        .save-btn { width: 100%; background-color: #d81b60; color: white; border: none; padding: 14px; font-size: 16px; font-weight: 500; border-radius: 4px; }
        .secure-checkout-badge { text-align: center; margin-bottom: 10px; color: #6c757d; font-size: 14px; }
        .secure-checkout-badge i { color: #28a745; }
        .trust-footer { text-align: center; padding: 20px 10px; background-color: #f8f9fa; }
        .trust-footer p { margin-bottom: 10px; font-size: 12px; color: #6c757d; }
        .trust-footer .policy-links a { color: #6c757d; text-decoration: none; margin: 0 8px; font-size: 12px; }
    </style>
</head>
<body>
    ...
    <i class="bi bi-arrow-left fs-4" style="color: white;"></i>
        ...
        <span class="header-brand"><?php echo $brandName; ?></span>
    </header>
        ...
        <section class="form-section">
            <form method="POST" action="address">
                <div class="form-floating mb-3">
                    <input type="text" class="form-control" id="name" name="name" placeholder="Full Name" value="<?php echo htmlspecialchars($saved_address['name'] ?? ''); ?>">
  ...
  numberInput.addEventListener('input', function () {
    ...
    if (this.value.length !== 10) {
      ...
      this.setCustomValidity(""); // clear error
                ...
                <div class="form-floating mb-3">
                    <input type="tel" class="form-control" id="pincode" name="pin" placeholder="Pincode" value="<?php echo htmlspecialchars($saved_address['pincode'] ?? ''); ?>">
                ...
                <div class="row g-3 mb-3">
                    <div class="col-6">
                        <div class="form-floating">
                            <input type="text" class="form-control" id="city" name="city" placeholder="City" value="<?php echo htmlspecialchars($saved_address['city'] ?? ''); ?>">
                    ...
                    <div class="col-6">
                        <div class="form-floating">
                            <select class="form-select" id="state" name="state">
                                <option value="" disabled <?php echo empty($saved_address['state']) ? 'selected' : ''; ?>>Select State</option>
                                <?php
                ...
                <div class="form-floating mb-3">
                     <input type="text" class="form-control" id="flat" name="flat" placeholder="House No., Building Name" value="<?php echo htmlspecialchars($saved_address['flat'] ?? ''); ?>">
                     <label for="flat">House No., Building Name</label>
                </div>
               
                <div class="form-floating mb-3">
                     <input type="text" class="form-control" id="area" name="area" placeholder="Road name, Area, Colony" value="<?php echo htmlspecialchars($saved_address['area'] ?? ''); ?>">
                ...
                <footer class="page-footer">
                    <div class="secure-checkout-badge">
                        <i class="bi bi-shield-lock-fill"></i> 100% Secure & Safe Payments
                    </div>
                    <button type="submit" class="save-btn">Save and Deliver Here</button>
                    <p class="text-center text-muted small mt-2">You can review your order before payment.</p>
                </footer>
            </form>
        </section>
        ...
        <footer class="trust-footer">
            ...
            <div class="policy-links">
                ...
                <a href="return-policy" target="_blank">Shipping & Return Policy</a>
            </div>
        </footer>
    </main>
</body>
